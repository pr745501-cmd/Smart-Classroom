...
import { DmChatService, FacultyContact, DmInboxUpdatePayload } from '../../../../core/services/dm-chat.service';
...
export class FacultyListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.dmChatService.getFacultyList().subscribe(data => {
      this.contacts = data;
      this.cd.detectChanges();
    });
  ...
  trackContact(_index: number, contact: FacultyContact): string {
    return contact._id;
  }
}
